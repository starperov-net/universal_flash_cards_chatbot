...
from app.tests.utils import TELEGRAM_USER_GOOGLE, TELEGRAM_USER_1, TELEGRAM_USER_2, TELEGRAM_USER_3

CONTEXT_en = tables.Context(
    name="English",
    name_alfa2="en"
)

CONTEXT_de = tables.Context(
    name="German",
    name_alfa2="de"
)

CONTEXT_uk = tables.Context(
    name="Ukrainian",
    name_alfa2="uk"
)

CONTEXT_ru = tables.Context(
    name="Russian",
    name_alfa2="ru"
)
...
ITEM_en_auto = tables.Item(
    author=USER_1.id,
    context=CONTEXT_en.id,
    text='auto'
)
ITEM_de_auto = tables.Item(
    author=USER_1.id,
    context=CONTEXT_de.id,
    text='auto'
)
ITEM_de_wagen = tables.Item(
    author=USER_GOOGLE.id,
    context=CONTEXT_de.id,
    text='wagen'
)
...
ITEM_uk_automobil = tables.Item(
    author=USER_1.id,
    context=CONTEXT_uk.id,
    text='автомобіль'
)

ITEM_ru_mashina = tables.Item(
    author=USER_1.id,
    context=CONTEXT_ru.id,
    text='машина'
)

ITEM_RELATION_1_ru_de = tables.ItemRelation(
    author=USER_1.id,
    item_1=ITEM_ru_mashina,
    item_2=ITEM_de_auto
)

ITEM_RELATION_1_uk_en = tables.ItemRelation(
    author=USER_1.id,
    item_1=ITEM_uk_automobil,
    item_2=ITEM_en_auto
)
