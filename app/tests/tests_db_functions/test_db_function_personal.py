...
class TestVerificationOfRecordedDataToDB(IsolatedAsyncioTestCase):
    ...
    async def test_add_user_db(self, telegram_user: aiogram.types.User) -> None:
    ...
    async def test_add_item_db(self) -> None:
        ...
        item: Item = await add_item_db(
            author=USER_1.id, context=CONTEXT_en.id, text=text
        )
...
class TestGetItemRelation(IsolatedAsyncioTestCase):
    ...
    @parameterized.expand(
        [
            (
                ...
                "машина",
                3,
                USER_CONTEXT_3_ru_de.context_1,
                USER_CONTEXT_3_ru_de.context_2,
            ),
            (
                "машина",
                None,
                1,
                USER_CONTEXT_1_uk_en.context_1,
                USER_CONTEXT_1_uk_en.context_2,
            ),
            (
                "автомобіль",
                "auto",
            ...
            (
                ...
                "автомобіль",
                2,
                USER_CONTEXT_2_uk_en.context_1,
                USER_CONTEXT_2_uk_en.context_2,
            ),
            (
                "машина",
                "auto",
    ...
    async def test_get_translated_text_db(
        ...
        context_1: Context.id,
        context_2: Context.id,
    ) -> None:
        answer: t.Optional[str] = await get_translated_text_db(
            word, telegram_user_id, context_1, context_2
        )
...
async def test_add_item_relation_db(user, item_uk, item_en) -> None:
    item_relation: ItemRelation = await add_item_relation_db(
        author=user.id, item_1=item_en.id, item_2=item_uk.id
    )
